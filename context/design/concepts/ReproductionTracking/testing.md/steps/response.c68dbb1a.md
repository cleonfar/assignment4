...
content_id: c68dbb1a522875bc1450ea43b202a6ccfc7cdcaa956a6d5d333d07e1a21d1e43
---

# response:

Here are the test cases for the `generateReport` action. I've included scenarios for creating new reports, adding to existing reports, and handling cases with no data or non-existent mothers.
...
Deno.test("ReproductionTrackingConcept - generateReport action", async (t) => {
  ...
  // Make OA1-1 weaned (1/3)
  await concept.recordWeaning({ offspringId: "offspring:OA1-1" as string });
  // Make OA1-2 dead (0/3 remaining alive for A1, but this just affects `isAlive`, not `survivedTillWeaning` status if previously true)
  // Re-evaluating `survivedTillWeaning` after `recordDeath`: `recordDeath` explicitly sets `survivedTillWeaning` to `false`.
  // So, if OA1-1 was weaned, and then died, it would be `isAlive: false` and `survivedTillWeaning: false`.
  ...
  // Make all A2 offspring weaned (3/3)
  ...
  const reportName2 = "MotherBSpecialReport";
  ...
  await t.step("should successfully generate a new report for a single target and date range", async () => {
    ...
    const expectedOffspringCount = 6; // 3 from A1, 3 from A2
    const expectedWeanedCount = 3; // OA1-1 was weaned, but then OA1-2 died. Re-check logic: OA1-1 weaned, then OA1-2 died. Total weaned: OA1-1, OA2-1, OA2-2, OA2-3. Total = 4. My math above was wrong.
    // OA1-1: weaned = true, isAlive = true
    // OA1-2: weaned = false, isAlive = false (died)
    // OA1-3: weaned = false, isAlive = true
    // OA2-1, OA2-2, OA2-3: all weaned = true, isAlive = true
    // Total offspring: 3 + 3 = 6
    // Weaned and alive: OA1-1, OA2-1, OA2-2, OA2-3 (4 total)
    // So 4/6 = 66.67%
    const expectedWeaningSurvival = ((4 / 6) * 100).toFixed(2) + '%';
    ...
    assertExists(generatedReport.results[0].includes(`Litters: 2`)); // A1 (Jan), A2 (Feb)
  ...
  await t.step("should add performance for a different target to an existing report", async () => {
    ...
    const expectedOffspringCount = 4; // 4 from B1
    const expectedWeanedCount = 2; // OB1-1, OB1-2
    const expectedWeaningSurvival = ((expectedWeanedCount / expectedOffspringCount) * 100).toFixed(2) + '%';
    ...
    assertExists(updatedReport.results[1].includes(`Litters: 1`)); // B1 (Jan)
  ...
  await t.step("should add a new performance entry for the same target but a different date range", async () => {
    ...
    const expectedOffspringCount = 3; // 3 from A2
    const expectedWeanedCount = 3; // OA2-1, OA2-2, OA2-3
    const expectedWeaningSurvival = ((expectedWeanedCount / expectedOffspringCount) * 100).toFixed(2) + '%';
    ...
    assertExists(updatedReport.results[initialResultsCount].includes(`Litters: 1`)); // Only A2
