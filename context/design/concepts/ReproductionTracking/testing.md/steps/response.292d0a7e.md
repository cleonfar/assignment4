---
timestamp: 'Fri Oct 17 2025 08:39:55 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251017_083955.d56033d9.md]]'
content_id: 292d0a7eb2c3fedb39ab65a61b3e5e2d5194919ac0494ef66862e51a8a91c3ae
---

# response:

To properly test the AI-related actions without making actual external API calls, we'll implement a mocking strategy for the `GoogleGenerativeAI` client. This involves:

1. **Modifying `ReproductionTrackingConcept`**: The constructor will now optionally accept an LLM client. If not provided (e.g., in production), it will initialize the real `GoogleGenerativeAI` client, falling back to a dummy client if `GEMINI_API_KEY` is missing. This allows us to inject a mock client during tests.
2. **Creating Mock LLM Clients**: In the test file, we'll create helper functions to generate mock LLM clients that return predefined JSON strings or throw errors, simulating different AI behaviors.
3. **Test Cases**:
   * **High/Low Performers**: Test the AI's ability to classify mothers as high or low performers based on generated report data.
   * **Caching (`_aiSummary`)**: Verify that `_aiSummary` returns a cached summary if one exists and doesn't call the LLM again.
   * **Regenerate (`regenerateAISummary`)**: Ensure `regenerateAISummary` always calls the LLM and overwrites any existing summary.
   * **Record Errors**: Test the AI's ability to identify "potential record errors" by manually inserting a report entry that implies inconsistent data (e.g., 150% weaning survival).
   * **LLM Response Validation**: Test how the concept handles invalid JSON or malformed JSON from the LLM.

First, let's update the `ReproductionTrackingConcept.ts` file to support dependency injection for the LLM client.
...
import { GoogleGenerativeAI } from "npm:@google-generative-ai"; // Ensure this is the correct import for the Deno environment
...
/**
 * Interface for a mockable LLM client.
 * This allows us to inject a test doubles in our tests.
 */
interface MockableLLMClient {
...
export default class ReproductionTrackingConcept {
  ...
  private llmClient: MockableLLMClient; // Use the mockable interface

  constructor(private readonly db: Db, llmClient?: MockableLLMClient) {
    ...
    if (llmClient) {
      this.llmClient = llmClient; // Use the provided mock client for testing
    } else {
      const apiKey = Deno.env.get("GEMINI_API_KEY");
      if (!apiKey) {
        console.warn("GEMINI_API_KEY not set. AI summary features will not work.");
        // Provide a dummy client that always errors out if no API key in prod
        this.llmClient = {
          getGenerativeModel: () => ({
        ...
        this.llmClient = new GoogleGenerativeAI(apiKey); // Use the real client in production
      }
    }
  }
  ...
  async recordLitter({
    ...
    if (duplicateLitter) {
      return {
        error:
          `A litter with mother ${motherId}, father ${
            fatherId || "none"
  ...
  async updateLitter({
    ...
    if (motherId !== undefined) {
      ...
      if (!existingMother) {
        ...
        if (addMotherResult.error) {
          return {
            error: `Failed to ensure new mother for litter exists: ${addMotherResult.error}`,
          };
        }
      }
  ...
  async recordWeaning({
    ...
    if (!existingOffspring.isAlive) {
      return {
        error: `Offspring with ID '${offspringId}' is not alive and cannot be weaned.`,
      };
    }
  ...
  async recordDeath({
    ...
    if (!existingOffspring.isAlive) {
      return {
        error: `Offspring with ID '${offspringId}' is already marked as deceased.`,
      };
    }

    // Corrected logic based on spec: only set isAlive to false.
   ...
   */
  async deleteReport({ reportName }: { reportName: string }): Promise<
    Empty | { error: string }
  > {
   ...
   * @throws {Error} If LLM response is invalid or an internal error occurs.
   */
  private async _callLLMAndGetSummary(report: Report): Promise<string> {
    // No need to check apiKey here, it's handled in the constructor now.
...
`;
  ...
  async _aiSummary({
    ...
    // If a summary already exists, return it immediately to avoid redundant AI calls.
    if (report.summary !== "") {
...
Now, let's add the new tests for AI-related actions in `ReproductionTrackingConcept.test.ts`. I will create a separate test file to keep the length manageable.

```typescript
// file: src/ReproductionTracking/ReproductionTrackingConcept.ai.test.ts
...
// --- Mocking infrastructure for LLM client ---
interface MockableLLMClient {
  getGenerativeModel(params: { model: string }): {
    generateContent(
      prompt: string,
...
// Function to create a mock LLM client that returns a predefined JSON string
const createMockLLMClient = (
  mockResponse: { text: () => string } | Error,
): MockableLLMClient => {
  return {
    getGenerativeModel: () => ({
...
// --- Test suite for AI-related actions ---

Deno.test("ReproductionTrackingConcept - AI Summary Actions", async (t) => {
  const [db, client] = await testDb();
  // We'll initialize the concept with specific mocks for each test step

  // Setup common test data for reports
  const motherHId = "mother:HighPerformer" as ID;
  const motherLId = "mother:LowPerformer" as ID;
  const motherMId = "mother:ModeratePerformer" as ID;
  const motherEId = "mother:ErrorCandidate" as ID; // For record error test

  await db.collection("ReproductionTracking.mothers").insertMany([
    { _id: motherHId, notes: "" },
    { _id: motherLId, notes: "" },
    { _id: motherMId, notes: "" },
    { _id: motherEId, notes: "" },
  ...
  // Manually insert reports for testing
  const reportsCollection: Collection<any> = db.collection(
    "ReproductionTracking.reports",
  );

  // Report for high performer
  const reportHighPerfName = "HighPerformerReport";
  await reportsCollection.insertOne({
    _id: reportHighPerfName,
    dateGenerated: new Date(),
    target: [motherHId],
    results: [
      `Performance for ${motherHId} (Jan 1, 2023 to Dec 31, 2023): Litters: 10, Offspring: 80, Weaning Survival: 95.00%`,
    ],
    summary: "", // Start with empty summary
  });

  // Report for low performer
  const reportLowPerfName = "LowPerformerReport";
  await reportsCollection.insertOne({
    _id: reportLowPerfName,
    dateGenerated: new Date(),
    target: [motherLId],
    results: [
      `Performance for ${motherLId} (Jan 1, 2023 to Dec 31, 2023): Litters: 2, Offspring: 5, Weaning Survival: 20.00%`,
    ],
    summary: "", // Start with empty summary
  });

  // Report for moderate performer
  const reportModPerfName = "ModeratePerformerReport";
  await reportsCollection.insertOne({
    _id: reportModPerfName,
    dateGenerated: new Date(),
    target: [motherMId],
    results: [
      `Performance for ${motherMId} (Jan 1, 2023 to Dec 31, 2023): Litters: 5, Offspring: 25, Weaning Survival: 75.00%`,
    ],
    summary: "", // Start with empty summary
  });

  // Report with a glaring record error
  const reportErrorName = "RecordErrorReport";
  await reportsCollection.insertOne({
    _id: reportErrorName,
    dateGenerated: new Date(),
    target: [motherEId],
    results: [
      `Performance for ${motherEId} (Jan 1, 2023 to Dec 31, 2023): Litters: 3, Offspring: 4, Weaning Survival: 150.00%`, // Obvious error
      `Performance for another:Normal (Jan 1, 2023 to Dec 31, 2023): Litters: 5, Offspring: 25, Weaning Survival: 80.00%`,
    ],
    summary: "",
  });

  // A report that will have a pre-existing summary for caching tests
  const reportCachedName = "CachedSummaryReport";
  const initialCachedSummary = JSON.stringify({
    highPerformers: [],
    lowPerformers: [],
    concerningTrends: [],
    averagePerformers: ["mother:CachedMom"],
    potentialRecordErrors: [],
    insights: "This is a pre-existing cached summary.",
  });
  await reportsCollection.insertOne({
    _id: reportCachedName,
    dateGenerated: new Date(),
    target: ["mother:CachedMom" as ID],
    results: [`Performance for mother:CachedMom: Litters: 5, Offspring: 20, Weaning Survival: 70.00%`],
    summary: initialCachedSummary, // Pre-populated summary
  });

  await t.step("should correctly classify high/low/average performers and identify record errors", async () => {
    const mockLLMResponse = JSON.stringify({
      highPerformers: [motherHId],
      lowPerformers: [motherLId],
      concerningTrends: [],
      averagePerformers: [motherMId],
      potentialRecordErrors: [motherEId],
      insights:
        `Key findings show ${motherHId} as a standout performer. ${motherLId} needs significant intervention. ` +
        `The entry for ${motherEId} shows an impossible weaning survival rate of 150%, which is a clear record error ` +
        `and warrants immediate investigation.`,
    });
    const mockLLM = createMockLLMClient({ text: () => mockLLMResponse });
    const concept = new ReproductionTrackingConcept(db, mockLLM);

    const result = await concept._aiSummary({ reportName: reportErrorName });
    assertExists(result.summary);
    assertEquals(result.error, undefined);

    const summary: any = JSON.parse(result.summary!);
    assertEquals(summary.highPerformers, [motherHId]);
    assertEquals(summary.lowPerformers, [motherLId]);
    assertEquals(summary.averagePerformers, [motherMId]);
    assertEquals(summary.potentialRecordErrors, [motherEId]);
    assertExists(summary.insights.includes("impossible weaning survival rate of 150%"));

    const updatedReport = await reportsCollection.findOne({ _id: reportErrorName });
    assertEquals(updatedReport?.summary, mockLLMResponse);
  });

  await t.step("should return cached summary without calling LLM if it already exists", async () => {
    // This mock will throw if called, proving it wasn't invoked by _aiSummary
    const mockLLM = createMockLLMClient(new Error("LLM should not be called for cached summary"));
    const concept = new ReproductionTrackingConcept(db, mockLLM);

    // Initial call to _aiSummary for a report that already has a summary
    const result = await concept._aiSummary({ reportName: reportCachedName });

    assertExists(result.summary);
    assertEquals(result.error, undefined);
    assertEquals(result.summary, initialCachedSummary, "Should return the existing cached summary");

    // Verify LLM was not called (no error thrown from mock)
  });

  await t.step("should call LLM and overwrite summary when regenerateAISummary is used", async () => {
    const newSummaryText = JSON.stringify({
      highPerformers: ["mother:CachedMom"],
      lowPerformers: [],
      concerningTrends: [],
      averagePerformers: [],
      potentialRecordErrors: [],
      insights: "This is a NEW regenerated summary.",
    });
    const mockLLM = createMockLLMClient({ text: () => newSummaryText });
    const concept = new ReproductionTrackingConcept(db, mockLLM);

    // Before regeneration, ensure old summary exists
    const oldReport = await reportsCollection.findOne({ _id: reportCachedName });
    assertExists(oldReport);
    assertEquals(oldReport.summary, initialCachedSummary);

    // Call regenerateAISummary
    const result = await concept.regenerateAISummary({ reportName: reportCachedName });

    assertExists(result.summary);
    assertEquals(result.error, undefined);
    assertEquals(result.summary, newSummaryText, "Should return the newly generated summary");

    // Verify the database was updated with the new summary
    const updatedReport = await reportsCollection.findOne({ _id: reportCachedName });
    assertEquals(updatedReport?.summary, newSummaryText);
  });

  await t.step("should return an error if LLM returns invalid JSON", async () => {
    const invalidJsonText = "This is not valid JSON.";
    const mockLLM = createMockLLMClient({ text: () => invalidJsonText });
    const concept = new ReproductionTrackingConcept(db, mockLLM);

    const result = await concept._aiSummary({ reportName: reportHighPerfName });

    assertExists(result.error);
    assertEquals(result.summary, undefined);
    assertExists(result.error.includes("Invalid JSON response from LLM"));
    assertExists(result.error.includes(invalidJsonText));
  });

  await t.step("should return an error if LLM returns JSON with incorrect structure", async () => {
    const malformedJson = JSON.stringify({
      notTheRightField: "some value",
      anotherField: 123,
    });
    const mockLLM = createMockLLMClient({ text: () => malformedJson });
    const concept = new ReproductionTrackingConcept(db, mockLLM);
    ...
    assertEquals(result.summary, undefined);
    assertExists(result.error.includes("Invalid JSON response from LLM"));
    assertExists(result.error.includes("Parsed JSON does not match expected structure or types."));
  });

  await t.step("should return an error if the report does not exist", async () => {
    const mockLLM = createMockLLMClient({ text: () => "{}" }); // LLM won't be called, but provide a dummy
    const concept = new ReproductionTrackingConcept(db, mockLLM);
    ...
    const regenResult = await concept.regenerateAISummary({ reportName: "NonExistentReport" });
    assertExists(regenResult.error);
    assertEquals(regenResult.summary, undefined);
    assertEquals(regenResult.error, "Report with name 'NonExistentReport' not found.");
  });
