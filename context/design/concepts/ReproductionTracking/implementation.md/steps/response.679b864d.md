---
timestamp: 'Thu Oct 16 2025 17:31:23 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251016_173123.bf27aa8d.md]]'
content_id: 679b864d9b808210d25fc051d815b6e8c6c52319c53462f49bab4cb15b382a40
---

# response:

You're absolutely right to catch that! The issue with `recordLitter` for cases where no father is given, and the subsequent duplicate check, stems from a subtle interplay between TypeScript's `undefined` for optional properties and MongoDB's handling of `null` vs. absent fields.

The original `Litter` interface used `fatherId?: ID`, which implies `ID | undefined`. When `fatherId` was not provided in the input, it was stored as `undefined`, meaning the `fatherId` field was simply *absent* from the MongoDB document. The duplicate check then used `{$exists: false}` to find such documents.

Your request clarifies the desired behavior:

1. **Store `null` for absent father:** If `fatherId` is not provided in the `recordLitter` input, it should be explicitly stored as `null` in the database, rather than making the field absent.
2. **Duplicate check for `null`:** The duplicate check should then compare `null` with `null` for litters without a father.

This requires a change to the `Litter` interface and the logic within both `recordLitter` and `updateLitter`.

Additionally, I noticed a discrepancy in the `Report` state definition regarding `target`. The concept spec defines `target` as `Set<String>`, but the TypeScript interface `Report` had `target: MotherId;` (singular). This has been updated to `targets: MotherId[];` to align with the specification's intent for a report to potentially track multiple target animals over time.

Here's the updated `ReproductionTrackingConcept` with these changes:
...
export default class ReproductionTrackingConcept {
  ...
  async recordLitter({
    ...
    const litterFilter: {
      motherId: MotherId;
      fatherId: ID | null; // Filter for ID or null
      birthDate: Date;
    } = {
