---
timestamp: 'Thu Oct 16 2025 17:22:13 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251016_172213.425f8298.md]]'
content_id: 26f834b309248e16141971974aa6e0326b126227bf8ea071cda207d4c3c98b31
---

# response:

You're encountering a classic race condition scenario in database operations. While your previous `findOne` check before `insertOne` was helpful for immediate feedback, it doesn't guarantee atomicity against concurrent requests. If two requests for the "same" litter (according to your uniqueness criteria) hit the `findOne` check simultaneously, both might find no existing litter and then both would proceed to `insertOne`, resulting in duplicates.

The robust solution for enforcing uniqueness in MongoDB (and most databases) is to use a **unique index**. This shifts the uniqueness enforcement to the database layer, which handles it atomically. If an `insertOne` (or `updateOne`) operation violates a unique index, MongoDB will throw a `MongoServerError` with error code `11000`. Your code can then catch this specific error and return a user-friendly message.

Here's the updated `ReproductionTrackingConcept` with the necessary changes:

1. **Unique Compound Index**: A unique compound index on `motherId`, `fatherId`, and `birthDate` is created for the `litters` collection. This index handles cases where `fatherId` is absent (stored as an omitted field) correctly, enforcing uniqueness across all combinations.
2. **`recordLitter` Action**:
   * The `findOne` check for duplicates **before** the insert has been removed to simplify the logic and fully rely on the unique index for atomicity.
   * The `insertOne` call is now wrapped in a `try-catch` block to specifically intercept `MongoServerError` with `code: 11000` (duplicate key error).
   * A user-friendly error message is returned when a duplicate is attempted.
3. **`updateLitter` Action**: Added `try-catch` to `updateLitter` as well, as an update operation could also violate a unique index if it changes fields that are part of the uniqueness constraint to values that already exist in another document.

```typescript
import { Collection, Db, MongoServerError } from "npm:mongodb"; // Import MongoServerError
...
export default class ReproductionTrackingConcept {
  ...
  constructor(private readonly db: Db) {
    ...
    // Initialize indexes on startup to ensure uniqueness constraints are active
  ...
  /**
   * Initializes unique indexes for collections to enforce data integrity.
   * This is crucial for preventing duplicate entries in race conditions.
   */
  private async initializeIndexes() {
    // Unique compound index for litters: ensures no two litters have the same
    // mother, father (or absence of father), and exact birth date.
    // MongoDB treats fields set to `undefined` in TS as absent from the document,
    // and handles such cases correctly in unique compound indexes.
    await this.litters.createIndex(
      { motherId: 1, fatherId: 1, birthDate: 1 },
      { unique: true, name: "unique_litter_combination" },
    );
  }
  ...
  async recordLitter({
    ...
    const newLitter: Litter = {
      ...
      // Store fatherId as undefined if not provided; MongoDB will omit the field.
      fatherId: fatherId ? (fatherId as ID) : undefined,
      birthDate,
      reportedLitterSize,
      notes,
    };
  ...
  async updateLitter({
    ...
    // Handle fatherId update: if explicitly provided as undefined, it means unset/remove the field.
    // If it's not provided in the `args` object, it won't be in `updateFields` and won't be changed.
    if (Object.prototype.hasOwnProperty.call(arguments[0], 'fatherId')) {
      if (fatherId === undefined) {
          // Explicitly set to undefined to remove the field from the document if it exists.
          updateFields.fatherId = undefined;
      } else {
    ...
    try {
        ...
        if (e instanceof MongoServerError && e.code === 11000) {
            // This error occurs if the update *itself* would create a duplicate
            // based on the unique index (e.g., changing motherId/fatherId/birthDate
            // to a combination that already exists for another litter).
            return { error: `Update would create a duplicate litter combination (motherId, fatherId, birthDate).` };
        }
        throw e;
    }
  }
