---
timestamp: 'Sat Oct 25 2025 20:10:53 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251025_201053.b4b48c4f.md]]'
content_id: 1a8c14909299da46675bd18d8f135a141d03d712eb56cd504972e4c4d83d7fb1
---
...
export default class ReproductionTrackingConcept {
  ...
  /**
   * **action** `updateOffspring (oldOffspringId: String, newOffspringId: String?, litterId: String?, sex: Enum?, notes: String?): (offspringID: String)`
   *
   * **requires** `oldOffspringId` exists. If `newOffspringId` is provided and different from `oldOffspringId`,
   * it must not already exist. If `litterId` is changed, the new litter must exist.
   * **effects** Finds the offspring by `oldOffspringId`. If `newOffspringId` is provided and different,
  ...
  async updateOffspring({
    ...
    // 1. Find the offspring by old ID
    const existingOffspring = await this.offspring.findOne({
      _id: oldOffspringId as ID,
    });
    if (!existingOffspring) {
      return { error: `Offspring with ID '${oldOffspringId}' not found.` };
    }
    ...
    if (finalNewOffspringId !== oldOffspringId as ID) {
      ...
      const newIdExists = await this.offspring.findOne({ _id: finalNewOffspringId });
      if (newIdExists) {
        return { error: `New offspring ID '${finalNewOffspringId}' already exists. Cannot rename.` };
      }
      ...
      await this.offspring.deleteOne({ _id: oldOffspringId as ID });
      ...
      if (result.matchedCount === 0) {
        ...
        return {
          error: `Offspring with ID '${oldOffspringId}' not found for update.`,
        };
      }
      return { offspringID: oldOffspringId as ID };
    }
