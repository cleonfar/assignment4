---
timestamp: 'Sat Oct 25 2025 17:48:47 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251025_174847.ea1cf81b.md]]'
content_id: 2c3f8624683480d4848aeedcf6fe3c80ec44380689c7d60d1a005dbf891bd2a0
---

# response:

To ensure that removing a mother also removes her associated litters and offspring, the `removeMother` action needs to be updated to perform a cascade deletion.

Here's the modified `removeMother` action:
...
export default class ReproductionTrackingConcept {
  ...
  async removeMother({
    ...
    const littersToDelete = await this.litters.find({ motherId: motherId as ID }).toArray();
    ...
    if (result.deletedCount === 0) {
      ...
      return { error: `Mother with ID '${motherId}' not found during final deletion.` };
    }
...
`;
  ...
  async _listMothers(): Promise<{ mother?: Mother[]; error?: string }> {
    try {
      const mothers = await this.mothers.find({}).toArray();
      // Ensure we return an array of objects matching the Mother interface structure
      return { mother: mothers.map(m => ({ _id: m._id, notes: m.notes })) };
    } catch (e: any) {
      return { error: `Failed to list mothers: ${e.message}` };
    }
  }

  /**
   * **query** `_listLitters (): (litter: Litter)`
   *
   * **requires** true
   * **effects** Returns a list of all litters.
   * @returns {{ litter?: Litter[]; error?: string }} An array of litter objects, or an error.
   */
  async _listLitters(): Promise<{ litter?: Litter[]; error?: string }> {
    try {
      const litters = await this.litters.find({}).toArray();
      return { litter: litters.map(l => ({
        _id: l._id,
        motherId: l.motherId,
        fatherId: l.fatherId,
      ...
      return { error: `Failed to list litters: ${e.message}` };
    }
  }
  ...
  async _listOffspringByLitter({
    ...
    try {
      const offspringList = await this.offspring.find({ litterId: litterId as ID }).toArray();
      return { offspring: offspringList.map(o => ({
        _id: o._id,
        litterId: o.litterId,
        sex: o.sex,
        notes: o.notes,
      ...
      return { error: `Failed to list offspring for litter '${litterId}': ${e.message}` };
    }
  }
}
````
