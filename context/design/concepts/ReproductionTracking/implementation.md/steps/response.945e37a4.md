---
timestamp: 'Thu Oct 16 2025 16:52:49 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251016_165249.4adc1f5a.md]]'
content_id: 945e37a47bb73f6a6eeb20e5a76854ae560e5875980ccc0c6b836ed17e9cc93e
---
...
export default class ReproductionTrackingConcept {
  ...
  async removeMother({
    ...
    // todo: check requires (mother exists)
    // todo: perform effects (remove mother)
  ...
  async recordLitter({
    ...
    const duplicateLitter = await this.litters.findOne({
      ...
      fatherId: fatherId ? (fatherId as ID) : null,
      birthDate: birthDate,
    });
    if (duplicateLitter) {
      return {
  ...
  async updateLitter({
    ...
    if (fatherId !== undefined) updateFields.fatherId = fatherId as ID;
  ...
  async updateOffspring({
    ...
    if (litterId !== undefined) {
      const existingLitter = await this.litters.findOne({ _id: litterId as ID });
      if (!existingLitter) {
        return { error: `New litter with ID '${litterId}' not found.` }; // No auto-creation for litter here
      }
  ...
  async generateReport({
    ...
    const generatedResults = [
      ...
      `Litters recorded: X`, // This would be calculated
      `Total offspring: Y`, // This would be calculated
    ...
    if (existingReport) {
      ...
      await this.reports.updateOne(
        { _id: name as ReportName },
        {
          ...
          $addToSet: { results: { $each: generatedResults } },
        },
      );
    } else {
      // Create new report
