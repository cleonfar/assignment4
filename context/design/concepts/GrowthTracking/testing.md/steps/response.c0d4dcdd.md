---
timestamp: 'Sat Oct 18 2025 17:34:14 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_173414.51d706e2.md]]'
content_id: c0d4dcdd4a2938b62ce0af2e8769fd538f63b9a7470419712e396b693d1ff4a7
---

# response:

You're encountering the same union type issue, but this time with the result of `_getReportByName` query. TypeScript can't assume that `dbReportResult` will always have the `report` property, as it could also be the `{ error: string }` type.

The solution is to apply the same type guard you successfully used for the `generateReport` action's return value.

Here's the corrected test file, specifically fixing the `_getReportByName` call:
...
Deno.test("GrowthTrackingConcept - generateReport action", async (t) => {
  ...
  await t.step("should update an existing report with a new animal", async () => {
    ...
    if ("error" in dbReportResult) { // Apply type guard here
      ...
      return; // Fail the test if the query returns an error unexpectedly
    }
