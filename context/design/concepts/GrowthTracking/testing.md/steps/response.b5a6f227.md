---
timestamp: 'Sat Oct 18 2025 18:39:47 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_183947.8c91af86.md]]'
content_id: b5a6f227789dde2be235861bfc756de3641e9d41dc03a500c3a878e395157f50
---

# response:

You're looking for some advanced test scenarios, especially to validate the AI's classification and summary capabilities. This is excellent for ensuring the concept truly delivers on its promise for smart insights.

I will add 3 distinct scenarios as new `Deno.test` blocks to the `src/GrowthTracking/GrowthTrackingConcept.test.ts` file, focusing on challenging the `aiSummary` action and `_getAiSummary` query.

**Scenario Breakdown:**

1. **High, Low, and Average Performers**: Create data for three animals that clearly demonstrate high, low, and average growth rates. The AI should correctly categorize them.
2. **Potential Record Error & Concerning Trend**: One animal with a weight record that is highly suspicious (e.g., negative weight, or impossibly high/low jump), and another with a concerning trend (e.g., significant weight loss, or very erratic changes).
3. **Report with No Data / Edge Cases for ADG**: An animal with only one weight record (ADG should be null), and another animal with records but whose average daily gain calculates to exactly zero. The AI should handle these gracefully.

To ensure the AI calls work in the test environment, I'll add a `Deno.env.set` for `GEMINI_API_KEY` at the top of the test file. Remember that for actual deployment, this key should be managed securely (e.g., via environment variables in your Deno Deploy project or a `.env` file).

Here's the updated test file, including these new scenarios:
...
// Set a dummy API key for testing purposes. In a real environment, this would come from .env or CI/CD secrets.
// This prevents the _callLLMAndGetSummary from throwing an error about a missing key during tests.
Deno.env.set("GEMINI_API_KEY", "test-api-key");

// Helper to determine if an object is an error object
function isError(
  response: Record<PropertyKey, unknown>, // Changed to PropertyKey for broader compatibility with record keys
...
// --- NEW TEST SCENARIOS FOR AI ACCURACY ---
Deno.test("GrowthTrackingConcept - AI Accuracy Scenarios", async (t) => {
  const [db, client] = await testDb();
  const concept = new GrowthTrackingConcept(db);

  const animalFast: ID = "animal:FastGrowth" as ID;
  const animalSlow: ID = "animal:SlowGrowth" as ID;
  const animalAvg: ID = "animal:AverageJoe" as ID;
  const animalErr: ID = "animal:Anomaly" as ID;
  const animalLoss: ID = "animal:WeightLoss" as ID;
  const animalZeroGain: ID = "animal:NoChange" as ID;
  const animalSingleRec: ID = "animal:OneRecord" as ID;

  const dates = {
    d1: new Date("2024-01-01T00:00:00.000Z"),
    d2: new Date("2024-01-05T00:00:00.000Z"),
    d3: new Date("2024-01-10T00:00:00.000Z"),
    d4: new Date("2024-01-15T00:00:00.000Z"),
    d5: new Date("2024-01-20T00:00:00.000Z"),
  };

  const reportAIDefinitive = "AIDefinitiveReport";

  // Setup data for high, low, average, error, loss, zero gain, single record
  await concept.recordWeight({ animal: animalFast, date: dates.d1, weight: 100, notes: "" });
  await concept.recordWeight({ animal: animalFast, date: dates.d3, weight: 130, notes: "" }); // High gain: 30kg in 9 days

  await concept.recordWeight({ animal: animalSlow, date: dates.d1, weight: 80, notes: "" });
  await concept.recordWeight({ animal: animalSlow, date: dates.d3, weight: 82, notes: "" }); // Low gain: 2kg in 9 days

  await concept.recordWeight({ animal: animalAvg, date: dates.d1, weight: 90, notes: "" });
  await concept.recordWeight({ animal: animalAvg, date: dates.d3, weight: 100, notes: "" }); // Average gain: 10kg in 9 days

  await concept.recordWeight({ animal: animalErr, date: dates.d1, weight: 50, notes: "" });
  await concept.recordWeight({ animal: animalErr, date: dates.d2, weight: 5000, notes: "Impossible weight" }); // Record Error: huge jump
  await concept.recordWeight({ animal: animalErr, date: dates.d3, weight: 55, notes: "" }); // Back to normal range

  await concept.recordWeight({ animal: animalLoss, date: dates.d1, weight: 70, notes: "" });
  await concept.recordWeight({ animal: animalLoss, date: dates.d3, weight: 60, notes: "" }); // Concerning trend: 10kg loss in 9 days

  await concept.recordWeight({ animal: animalZeroGain, date: dates.d1, weight: 60, notes: "" });
  await concept.recordWeight({ animal: animalZeroGain, date: dates.d3, weight: 60, notes: "" }); // Zero gain

  await concept.recordWeight({ animal: animalSingleRec, date: dates.d1, weight: 75, notes: "" }); // Only one record

  // Generate a single report for all animals
  await concept.generateReport({ animal: animalFast, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalSlow, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalAvg, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalErr, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalLoss, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalZeroGain, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });
  await concept.generateReport({ animal: animalSingleRec, startDateRange: dates.d1, endDateRange: dates.d5, reportName: reportAIDefinitive });

  await t.step("AI should accurately classify animals into performance categories and identify errors", async () => {
    const aiResult = await concept.aiSummary({ reportName: reportAIDefinitive });
    if (isError(aiResult)) {
      throw new Error(`AI summary failed: ${aiResult.error}`);
    }

    const parsedSummary = JSON.parse(aiResult.summary);

    // Validate high performers
    assertArrayIncludes(parsedSummary.highPerformers, [animalFast], "FastGrowth should be a high performer");
    assertEquals(parsedSummary.highPerformers.length, 1, "Only FastGrowth should be a high performer");

    // Validate low performers
    assertArrayIncludes(parsedSummary.lowPerformers, [animalSlow], "SlowGrowth should be a low performer");
    assertEquals(parsedSummary.lowPerformers.length, 1, "Only SlowGrowth should be a low performer");

    // Validate concerning trends (weight loss, zero gain, or potentially single record lacking context)
    assertArrayIncludes(parsedSummary.concerningTrends, [animalLoss, animalZeroGain], "WeightLoss and NoChange should be concerning trends");
    assertEquals(parsedSummary.concerningTrends.length, 2, "Only WeightLoss and NoChange should be concerning trends");

    // Validate average performers
    assertArrayIncludes(parsedSummary.averagePerformers, [animalAvg], "AverageJoe should be an average performer");
    assertEquals(parsedSummary.averagePerformers.length, 1, "Only AverageJoe should be an average performer");

    // Validate potential record errors
    assertArrayIncludes(parsedSummary.potentialRecordErrors, [animalErr], "Anomaly should be flagged as a potential record error");
    assertEquals(parsedSummary.potentialRecordErrors.length, 1, "Only Anomaly should be a potential record error");

    // Validate that the animal with only one record is classified (likely not as average, but maybe low or concerning by AI)
    // The prompt says "Every animal in the report must be classified". Let's check if singleRec is in ANY category.
    const allClassifiedAnimals = new Set([
    ...
    assertArrayIncludes(Array.from(allClassifiedAnimals), [animalSingleRec], "Animal with one record should be classified in some category");

    // Validate insights
    assertExists(parsedSummary.insights, "Insights should be present");
    assertNotEquals(parsedSummary.insights.length, 0, "Insights should not be empty");
    // You could add more sophisticated regex or keyword checks here for specific insights if needed,
    // but for non-deterministic AI, checking existence and non-emptiness is a good start.
  });

  await client.close();
});

Deno.test("GrowthTrackingConcept - AI handling of empty/single record reports", async (t) => {
  const [db, client] = await testDb();
  const concept = new GrowthTrackingConcept(db);

  const animalNoRec: ID = "animal:NoRecords" as ID;
  const animalOneRec: ID = "animal:SingleRecord" as ID;
  const reportEmpty = "EmptyReport";
  const reportSingle = "SingleRecordReport";

  // Setup: Animal with no records, animal with one record
  // No records for animalNoRec (it won't even be in the animals collection)
  await concept.recordWeight({ animal: animalOneRec, date: new Date("2024-02-01"), weight: 100, notes: "" });

  // Generate a report for an animal that doesn't exist in the system
  // This should be handled by `generateReport` itself, not reaching AI summary
  await t.step("generateReport should return error for non-existent animal, preventing AI call", async () => {
    const result = await concept.generateReport({
      animal: animalNoRec,
      startDateRange: new Date("2024-01-01"),
      endDateRange: new Date("2024-02-28"),
      reportName: reportEmpty,
    });
    if (!isError(result)) {
      throw new Error("Expected error for non-existent animal, but succeeded.");
    }
    assertEquals(result.error, `Animal with ID ${animalNoRec} not found. Cannot generate report.`);
  });

  // Generate a report for an animal with no records within the date range (AI should still process)
  await concept.recordWeight({ animal: animalNoRec, date: new Date("2024-03-01"), weight: 50, notes: "" }); // Add one record outside range
  const reportEmptyResult = await concept.generateReport({
    animal: animalNoRec,
    startDateRange: new Date("2024-01-01"),
    endDateRange: new Date("2024-02-28"),
    reportName: reportEmpty,
  });
  if (isError(reportEmptyResult)) throw new Error(`Setup failed for ${reportEmpty}: ${reportEmptyResult.error}`);
  assertEquals(reportEmptyResult.report.results[0].averageDailyGain, null, "ADG should be null for no records in range");
  assertEquals(reportEmptyResult.report.results[0].recordedWeights.length, 0, "No recorded weights in range");


  await t.step("AI should handle report with animal having no records in range", async () => {
    const aiResult = await concept.aiSummary({ reportName: reportEmpty });
    if (isError(aiResult)) {
      throw new Error(`AI summary failed: ${aiResult.error}`);
    }
    const parsedSummary = JSON.parse(aiResult.summary);
    // AI should categorize `animalNoRec` somewhere, likely 'concerningTrends' or 'lowPerformers'
    const allClassifiedAnimals = new Set([
      ...parsedSummary.highPerformers, ...parsedSummary.lowPerformers,
      ...parsedSummary.concerningTrends, ...parsedSummary.averagePerformers,
      ...parsedSummary.potentialRecordErrors,
    ]);
    assertArrayIncludes(Array.from(allClassifiedAnimals), [animalNoRec], "Animal with no records in range should be classified");
    // Assert it's in a relevant category for lack of data or poor performance
    assertArrayIncludes(parsedSummary.concerningTrends, [animalNoRec], "Animal with no records in range should be in concerningTrends (by AI interpretation)");
    assertNotEquals(parsedSummary.insights.length, 0, "Insights should not be empty for this case");
  });

  // Generate a report for an animal with exactly one record
  const reportSingleResult = await concept.generateReport({
    animal: animalOneRec,
    startDateRange: new Date("2024-02-01"),
    endDateRange: new Date("2024-02-01"),
    reportName: reportSingle,
  });
  if (isError(reportSingleResult)) throw new Error(`Setup failed for ${reportSingle}: ${reportSingleResult.error}`);
  assertEquals(reportSingleResult.report.results[0].averageDailyGain, null, "ADG should be null for single record");
  assertEquals(reportSingleResult.report.results[0].recordedWeights.length, 1, "One recorded weight");


  await t.step("AI should handle report with animal having only one record", async () => {
    const aiResult = await concept.aiSummary({ reportName: reportSingle });
    if (isError(aiResult)) {
      throw new Error(`AI summary failed: ${aiResult.error}`);
    }
    const parsedSummary = JSON.parse(aiResult.summary);
    ...
    assertArrayIncludes(Array.from(allClassifiedAnimals), [animalOneRec], "Animal with one record should be classified");
    // AI might put it in average, low, or even concerning if it interprets 'null ADG' as a lack of definable growth
    assertArrayIncludes(parsedSummary.averagePerformers, [animalOneRec], "Animal with one record should be in averagePerformers (by AI interpretation)");
    assertNotEquals(parsedSummary.insights.length, 0, "Insights should not be empty for this case");
  });
