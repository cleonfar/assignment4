---
timestamp: 'Sat Oct 18 2025 17:49:33 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_174933.82358f8f.md]]'
content_id: c56b6b0d3fdd1c53532035b5f33635e4e438fe0e0e2ee1a367b76ca3b7942783
---

# response:

You're encountering the same TypeScript union type issue, but for the `_getReportByName` query within your `renameReport` and `deleteReport` tests.

The `_getReportByName` function can return either `{ report: ReportDoc }` or `{ error: string }`. When you access `oldReport.error` or `deletedReportCheck.error` directly, TypeScript doesn't know for sure that the `error` property exists on `oldReport` or `deletedReportCheck`, hence the error.

The solution is to apply the same type guard (`if ("error" in result)`) for the results of `_getReportByName` calls in these tests, mirroring how you handled `generateReport`'s return value.
...
Deno.test("GrowthTrackingConcept - renameReport action", async (t) => {
  ...
  await t.step("should successfully rename an existing report", async () => {
    ...
    if (!("error" in oldReportCheck)) { // Expected an error because the report should be gone
    ...
    if ("error" in newReportCheck) { // Expected success here
...
Deno.test("GrowthTrackingConcept - deleteReport action", async (t) => {
  ...
  await t.step("should successfully delete an existing report", async () => {
    ...
    if (!("error" in deletedReportCheck)) { // Expected an error because the report should be gone
