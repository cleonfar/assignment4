...
content_id: 224cbeb3d32b8e38ebdfc3f48c3dec21ebab860ae6b533d2cbb14a2c0ad9bc95
---
...
Deno.test("GrowthTrackingConcept - generateReport action", async (t) => {
  ...
  await t.step("should create a new report for an animal with records in range", async () => {
    ...
    assertExists((result as { report: unknown }).report, "Expected report to be returned");
  ...
  await t.step("should update an existing report with a new animal", async () => {
    ...
    assertEquals(dbReportResult.error, undefined);
    const dbReport = (dbReportResult as { report: typeof report }).report; // Use type of local report for assertion
  ...
  await t.step("should return an error if animal ID does not exist in the system at all", async () => {
    ...
    assertEquals((result as { error: string }).error, `Animal with ID ${nonExistentAnimal} not found. Cannot generate report.`);
  });
  ...
  await t.step("should return an error if animal ID is missing", async () => {
    ...
    assertNotEquals(result, {}, "Expected an error object");
    assertExists((result as { error: string }).error);
    assertEquals((result as { error: string }).error, "Animal ID, report name, start date, and end date are required.");
  });
  ...
  await t.step("should return an error if startDateRange is invalid", async () => {
    ...
    assertNotEquals(result, {}, "Expected an error object");
    assertExists((result as { error: string }).error);
    assertEquals((result as { error: string }).error, "Invalid date range provided.");
  });
