---
timestamp: 'Sat Oct 18 2025 17:32:36 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_173236.19e5f778.md]]'
content_id: bf1a496961ab2270e8f4a65b0509d6b2fac200b49844dfe49f6551d16022d869
---

# response:

You are encountering a classic TypeScript error with union types. The return type of `generateReport` is `ReportDoc | { error: string }`. When you directly try to access `result.error`, TypeScript complains because it doesn't know for sure if `result` is the `ReportDoc` type (which doesn't have an `error` property) or the error type.

To fix this, you need to use a **type guard** to narrow down the type of `result` before accessing its properties.

Here's how to correct the `generateReport` test cases:

1. **For expected success**: Check if the `'error'` property is *not* present in the `result` object. If it's not, TypeScript can then infer that `result` must be of type `{ report: ReportDoc }`.
2. **For expected error**: Check if the `'error'` property *is* present in the `result` object. If it is, TypeScript can infer that `result` must be of type `{ error: string }`.

Let's refactor the `generateReport` test suite with these type guards:
...
Deno.test("GrowthTrackingConcept - generateReport action", async (t) => {
  ...
  await t.step("should create a new report for an animal with records in range", async () => {
    ...
    if ("error" in result) {
      assertNotEquals(result.error, undefined, `Expected no error, but got: ${result.error}`);
      return; // Early exit if unexpected error
    }
  ...
  await t.step("should update an existing report with a new animal", async () => {
    ...
    if ("error" in dbReportResult) {
      assertNotEquals(dbReportResult.error, undefined, `Expected no error getting report: ${dbReportResult.error}`);
      return;
    }
    const dbReport = dbReportResult.report; // Safely inferred
  ...
  await t.step("should return an error if animal ID does not exist in the system at all", async () => {
    ...
    if (!("error" in result)) {
      assertNotEquals(result, {}, "Expected an error object but got success.");
      return; // Early exit if unexpected success
    }
