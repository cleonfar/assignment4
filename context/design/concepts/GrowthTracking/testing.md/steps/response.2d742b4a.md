---
timestamp: 'Sat Oct 18 2025 17:38:03 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_173803.d3e03878.md]]'
content_id: 2d742b4abf12c78baccf4b44f7a25220597ed08708f9d1ab2663ab79cd424790
---

# response:

The error "Argument of type '{}' is not assignable to parameter of type '{ report: ReportDoc; }'." occurs because of how TypeScript's type inference interacts with `assertNotEquals` when dealing with union types.

In your previous test, for error cases, you had:

```typescript
    if (!("error" in result)) { // If 'error' is NOT in result, it's considered a success
      assertNotEquals(result, {}, "Expected an error object but got success."); // THIS LINE IS PROBLEMATIC
      return;
    }
    // ... then access result.error
```

The issue is that for `generateReport`, a successful result is `Promise<{ report: ReportDoc }>`.

1. When `!("error" in result)` is true, TypeScript narrows `result` to `{ report: ReportDoc }`.
2. Then, `assertNotEquals(result, {}, ...)` is called. The `assertNotEquals` function expects both its `actual` and `expected` arguments to be of the same type `T`.
3. TypeScript infers `T` from the first argument (`result`), making `T` equal to `{ report: ReportDoc }`.
4. It then tries to assign the second argument (`{}`) to `T` (`{ report: ReportDoc }`). Since `{}` does not have a `report` property, it's not assignable, leading to the error.

To fix this, we need to adjust the error handling logic to be more explicit and use `throw new Error` for unexpected outcomes, which clearly signals a test failure and avoids these type inference conflicts.

Here's the corrected test file. The main changes are:

* For test steps expecting **success**, if `error` is found in the result, `throw new Error`.
* For test steps expecting **error**, if `error` is *not* found in the result, `throw new Error`.

This clarifies the control flow for TypeScript and correctly handles unexpected results.
