...
content_id: 6d76dd7af04d7a21ad7bd9a2d632fbd069c157b048160a05532a88c11b1fd094
---

# file: src/HerdGrouping/HerdGroupingConcept.test.ts (Updated with more logging)
...
Deno.test("HerdGroupingConcept", async (t) => {
  ...
  await t.step("moveAnimal: should *not* return error if animal already in target herd, and ensure it's still there", async () => {
    ...
    console.log(`HERD_B before test: ${JSON.stringify(herdBComposition.animals)}`);
    ...
    console.log(`HERD_A before move: ${JSON.stringify(herdAComposition.animals)}`);
  ...
  await t.step("splitHerd: should split animals from one herd to another", async () => {
    ...
    let initialHerdA = await concept._viewComposition({ herdName: HERD_A });
    if ("error" in initialHerdA) throw new Error(`_viewComposition failed unexpectedly: ${initialHerdA.error}`);
    console.log(`HERD_A before split: ${JSON.stringify(initialHerdA.animals)}`);
    assertArrayIncludes(initialHerdA.animals, [ANIMAL_2, ANIMAL_4, ANIMAL_5, ANIMAL_6]);
    assertEquals(initialHerdA.animals.length, 4, "HERD_A should have 4 animals before splitHerd"); // Check length for robustness
    ...
    console.log(`HERD_C before split: ${JSON.stringify(initialHerdC.animals)}`);
  ...
  await t.step("splitHerd: should *not* return error if any animal not in source (lenient behavior)", async () => {
    console.log("\n--- Test: splitHerd: should *not* return error if any animal not in source ---");
    // Pre-test state: HERD_A: [ANIMAL_2, ANIMAL_6], HERD_C: [ANIMAL_4, ANIMAL_5] (from previous step)

    // ANIMAL_1 is not in HERD_A. ANIMAL_4 is not in HERD_A (it's in HERD_C). ANIMAL_6 IS in HERD_A.
    const animalsToSplit = [ANIMAL_1, ANIMAL_4, ANIMAL_6] as Array<ID>;

    let initialHerdA = await concept._viewComposition({ herdName: HERD_A });
    if ("error" in initialHerdA) throw new Error(`_viewComposition failed unexpectedly: ${initialHerdA.error}`);
    console.log(`HERD_A before lenient split: ${JSON.stringify(initialHerdA.animals)}`); // Expect: [ANIMAL_2, ANIMAL_6]

    let initialHerdC = await concept._viewComposition({ herdName: HERD_C });
    if ("error" in initialHerdC) throw new Error(`_viewComposition failed unexpectedly: ${initialHerdC.error}`);
    console.log(`HERD_C before lenient split: ${JSON.stringify(initialHerdC.animals)}`); // Expect: [ANIMAL_4, ANIMAL_5]
    ...
    // Verify HERD_A: ANIMAL_6 should be removed, ANIMAL_1 & ANIMAL_4 were never there.
    const finalHerdA = await concept._viewComposition({ herdName: HERD_A });
    if ("error" in finalHerdA) throw new Error(`_viewComposition failed unexpectedly: ${finalHerdA.error}`);
    assertEquals(finalHerdA.animals.includes(ANIMAL_1), false);
    assertEquals(finalHerdA.animals.includes(ANIMAL_4), false);
    assertEquals(finalHerdA.animals.includes(ANIMAL_6), false); // ANIMAL_6 should be gone from HERD_A
    assertArrayIncludes(finalHerdA.animals, [ANIMAL_2]); // Only ANIMAL_2 should remain
    console.log(`HERD_A after lenient split: ${JSON.stringify(finalHerdA.animals)}`); // Expect: [ANIMAL_2]


    // Verify HERD_C: ANIMAL_1, ANIMAL_4, ANIMAL_5 (from previous state), ANIMAL_6 should be there.
    // ANIMAL_4 & ANIMAL_5 were already there. ANIMAL_1 & ANIMAL_6 should be added.
    const finalHerdC = await concept._viewComposition({ herdName: HERD_C });
    if ("error" in finalHerdC) throw new Error(`_viewComposition failed unexpectedly: ${finalHerdC.error}`);
    assertArrayIncludes(finalHerdC.animals, [ANIMAL_1, ANIMAL_4, ANIMAL_5, ANIMAL_6]);
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_1).length, 1, "ANIMAL_1 should appear once in target herd");
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_4).length, 1, "ANIMAL_4 should appear once in target herd");
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_5).length, 1, "ANIMAL_5 should appear once in target herd");
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_6).length, 1, "ANIMAL_6 should appear once in target herd");
    console.log(`HERD_C after lenient split: ${JSON.stringify(finalHerdC.animals)}`); // Expect: [ANIMAL_1, ANIMAL_4, ANIMAL_5, ANIMAL_6]
  });

  await t.step("splitHerd: should *not* return error if any animal already in target, and ensure all are present once (no-op scenario)", async () => {
    console.log("\n--- Test: splitHerd: should *not* return error if any animal already in target (no-op scenario) ---");
    // State after previous test: HERD_A: [ANIMAL_2], HERD_C: [ANIMAL_1, ANIMAL_4, ANIMAL_5, ANIMAL_6]
    ...
    console.log(`HERD_C before no-op split: ${JSON.stringify(herdCComposition.animals)}`);
    assertArrayIncludes(herdCComposition.animals, [ANIMAL_1, ANIMAL_4, ANIMAL_5, ANIMAL_6]);

    let herdAComposition = await concept._viewComposition({ herdName: HERD_A });
    if ("error" in herdAComposition) throw new Error(`_viewComposition failed: ${herdAComposition.error}`);
    console.log(`HERD_A before no-op split: ${JSON.stringify(herdAComposition.animals)}`);
    assertArrayIncludes(herdAComposition.animals, [ANIMAL_2]);

    // Try to split [ANIMAL_4, ANIMAL_6] from HERD_A to HERD_C
    // In this scenario:
    // ANIMAL_4 is NOT in HERD_A (source) - it's in HERD_C (target)
    // ANIMAL_6 is NOT in HERD_A (source) - it's in HERD_C (target)
    // This will effectively be a no-op on HERD_A (nothing to pull), and a no-op on HERD_C ($addToSet won't add duplicates).
    ...
    assertEquals("error" in result, false); // Should not return an error due to leniency
    if ("error" in result) {
      throw new Error(`splitHerd failed unexpectedly: ${result.error}`);
    }
    ...
    assertArrayIncludes(finalHerdA.animals, [ANIMAL_2]); // Only ANIMAL_2 should remain
    console.log(`HERD_A after no-op split: ${JSON.stringify(finalHerdA.animals)}`);


    // Verify HERD_C is unchanged (ANIMAL_4, ANIMAL_6 already there, no duplicates added)
    const finalHerdC = await concept._viewComposition({ herdName: HERD_C });
    if ("error" in finalHerdC) throw new Error(`_viewComposition failed: ${finalHerdC.error}`);
    assertArrayIncludes(finalHerdC.animals, [ANIMAL_1, ANIMAL_4, ANIMAL_5, ANIMAL_6]);
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_1).length, 1);
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_4).length, 1);
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_5).length, 1);
    assertEquals(finalHerdC.animals.filter(a => a === ANIMAL_6).length, 1);
    console.log(`HERD_C after no-op split: ${JSON.stringify(finalHerdC.animals)}`);
  });
