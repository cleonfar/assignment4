...
content_id: ea9871e1e2c2f25ddc20747ba75e8351b9d1c7274808743676205687df03acb1
---

# file: src/HerdGrouping/HerdGroupingConcept.ts (Updated for Strict Split Source)
...
export default class HerdGroupingConcept {
  ...
  async moveAnimal(
    ...
    // The check for `animal` being in `targetHerd.members` is intentionally omitted here.
   ...
   * effects The specified `animalsToMove` are removed from `sourceHerd` and added to `targetHerd`.
   */
  async splitHerd(
    ...
    const sourceHerd = await this.groups.findOne({ _id: sourceHerdName });
    const targetHerd = await this.groups.findOne({ _id: targetHerdName }); // Check for targetHerd's existence
    ...
    if (!targetHerd) {
      return { error: `Target herd '${targetHerdName}' not found.` }; // Target must exist
    }
    if (targetHerd.isArchived) {
      return { error: `Target herd '${targetHerdName}' is archived.` };
    }

    // RE-INTRODUCING THIS STRICT PRECONDITION CHECK
    ...
    // The check for `animalsToMove` being in `targetHerd.members` is intentionally omitted here.
...
The tests for `splitHerd` need significant refactoring to accommodate the new strictness for the source herd, while still testing the "lenient target" behavior. I'll re-enable the logging to help trace.
