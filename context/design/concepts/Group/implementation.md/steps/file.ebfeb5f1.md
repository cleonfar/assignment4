...
content_id: ebfeb5f1228a57fa0b28d5d4c5fc2ef931a7a941a14f6156e9a911e9b4dd51fd
---
...
export default class HerdGroupingConcept {
  ...
  async splitHerd(
    ...
    if (!targetHerd) {
      ...
      // No need to re-fetch targetHerd's full document here, as its _id (targetHerdName) is sufficient for the update.
      // createHerd ensures it's unarchived.
    ...
    try {
      ...
      // Add to target herd (which either existed or was just created)
      await this.groups.updateOne(
        { _id: targetHerdName },
