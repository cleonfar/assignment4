---
timestamp: 'Sun Nov 02 2025 01:04:53 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251102_010453.fa1d1d3c.md]]'
content_id: d37223e7205c6a3816a7ee342165a16c86a7130a814e15d36adcf46931af1682
---
...
import { AnimalIdentity, Requesting, UserAuthentification } from "@concepts"; // Corrected import
...
export const UserRegisterRequest: Sync = ({ request, username, password }) => ({
  when: actions(
    [Requesting.request, {
      path: "/UserAuthentification/register",
      username,
      password,
    }, { request }], // Corrected path
  ),
  then: actions(
...
export const UserRegisterResponse: Sync = ({ request, user }) => ({
  when: actions(
    [Requesting.request, { path: "/UserAuthentification/register" }, {
      request,
    }], // Corrected path
...
export const UserLogoutRequest: Sync = ({ request, token }) => ({
  when: actions(
    [Requesting.request, { path: "/UserAuthentification/logout", token }, {
      request,
    }], // Corrected path
  ),
  then: actions(
