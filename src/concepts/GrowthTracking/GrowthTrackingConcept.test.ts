...
Deno.test("GrowthTrackingConcept - recordWeight action", async (t) => {
  ...
  await t.step(
    "should successfully record a weight for a new animal",
    async () => {
      ...
      assertEquals(
        result,
        {},
        "Expected successful recordWeight to return an empty object",
      );

      const animalDoc = await db.collection("GrowthTracking.animals").findOne({
        _id: animalA,
      });
      assertExists(
        animalDoc,
        "Animal document should exist after recording weight",
      );
      assertEquals(
        animalDoc.weightRecords.length,
        1,
        "Should have one weight record",
      );
  ...
  await t.step(
    "should successfully record another weight for an existing animal",
    async () => {
      const result = await concept.recordWeight({
        animal: animalA,
        date: date2,
      ...
      assertEquals(
        animalDoc.weightRecords.length,
        2,
        "Should now have two weight records",
      );
  ...
  await t.step(
    "should record a weight with empty notes if not provided",
    async () => {
      const result = await concept.recordWeight({
        animal: animalB,
        date: date1,
      ...
      assertEquals(
        animalDoc.weightRecords.length,
        1,
        "Should have one weight record for animal B",
      );
      assertObjectMatch(animalDoc.weightRecords[0], {
        date: date1,
        weight: 50,
      ...
      const updatedAnimalDoc = await db.collection("GrowthTracking.animals")
        .findOne({ _id: animalB });
      assertEquals(
        updatedAnimalDoc!.weightRecords[1].notes,
        "",
        "Notes should be an empty string if undefined",
      );
    },
  );

  await t.step(
    "should allow recording multiple weights for the same animal on the same date",
    async () => {
      ...
      assertExists(
        animalDoc,
        "Animal document should exist after recording multiple weights",
      );
      assertEquals(
        animalDoc.weightRecords.length,
        4,
        "Should now have four weight records for animal A",
      );
      ...
      const recordsOnDate3 = animalDoc.weightRecords.filter((
        r: (typeof animalDoc.weightRecords)[number],
      ) => r.date.getTime() === date3.getTime());
      assertEquals(
        recordsOnDate3.length,
        2,
        "Should have two records on date3",
      );
      assertObjectMatch(recordsOnDate3[0], {
        date: date3,
        weight: 110,
        notes: "Morning weigh-in",
      });
      assertObjectMatch(recordsOnDate3[1], {
        date: date3,
        weight: 112,
        notes: "Evening weigh-in",
      });
  ...
  await t.step("should return an error if animal ID is missing", async () => {
    ...
    assertExists(
      ...
      "Expected error property in result",
    );
    assertEquals(
      (result as { error: string }).error,
      "Animal ID, date, and weight are required.",
    );
  });
...
Deno.test("GrowthTrackingConcept - removeWeightRecord action", async (t) => {
  ...
  await t.step(
    "should successfully remove an existing weight record",
    async () => {
      const initialAnimalCDoc = await db.collection("GrowthTracking.animals")
        .findOne({ _id: animalC });
      assertEquals(
        initialAnimalCDoc?.weightRecords.length,
        3,
        "Animal C should have 3 records initially",
      );

      const result = await concept.removeWeightRecord({
        animal: animalC,
        date: dateC2,
      });
      ...
      assertEquals(
        result,
        {},
        "Expected successful removeWeightRecord to return an empty object",
      );

      const animalCDoc = await db.collection("GrowthTracking.animals").findOne({
        _id: animalC,
      });
      assertExists(animalCDoc, "Animal C document should still exist");
      assertEquals(
        animalCDoc.weightRecords.length,
        2,
        "Animal C should now have 2 weight records",
      );

      const remainingDates = animalCDoc.weightRecords.map((
        r: (typeof animalCDoc.weightRecords)[number],
      ) => r.date.getTime());
      assertEquals(
        remainingDates.includes(dateC1.getTime()),
        true,
        "Record for dateC1 should remain",
      );
      assertEquals(
        remainingDates.includes(dateC2.getTime()),
        false,
        "Record for dateC2 should be removed",
      );
      assertEquals(
        remainingDates.includes(dateC3.getTime()),
        true,
        "Record for dateC3 should remain",
      );
    },
  );

  await t.step(
    "should return an error if the weight record does not exist for an existing animal",
    async () => {
      const animalCDocBefore = await db.collection("GrowthTracking.animals")
        .findOne({ _id: animalC });
      ...
      const animalCDocAfter = await db.collection("GrowthTracking.animals")
        .findOne({ _id: animalC });
      assertEquals(
  ...
  await t.step(
    "should return an error if the animal does not exist",
    async () => {
      const result = await concept.removeWeightRecord({
  ...
  await t.step("should return an error if animal ID is missing", async () => {
    ...
    assertEquals(
      (result as { error: string }).error,
      "Animal ID and date are required.",
    );
  });
...
Deno.test("GrowthTrackingConcept - generateReport action", async (t) => {
  ...
  await t.step(
    "should create a new report for an animal with records in range",
    async () => {
  ...
  await t.step(
    "should update an existing report with a new animal",
    async () => {
      ...
      assertObjectMatch(animalFResult!, {
        ...
        averageDailyGain:
          (5 /
            ((d_feb10.getTime() - d_feb1.getTime()) / (1000 * 60 * 60 * 24))), // 5/9
      });

      // Verify the report in the database
      const dbReportResult = await concept._getReportByName({
        reportName: reportName1,
      });
      if (isError(dbReportResult)) { // Apply type guard here
        throw new Error(
          `Expected no error getting report, but got: ${dbReportResult.error}`,
        );
      }
  ...
  await t.step(
    "should update an existing report with updated data for an existing animal",
    async () => {
  ...
  await t.step(
    "should create a new report for an animal with no records in the specified range",
    async () => {
  ...
  await t.step(
    "should create a new report for an animal with only one record in range (ADG should be null)",
    async () => {
  ...
  await t.step(
    "should return an error if animal ID does not exist in the system at all",
    async () => {
      const result = await concept.generateReport({
        animal: nonExistentAnimal,
        startDateRange: d_jan1,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for non-existent animal, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
      assertEquals(
        result.error,
        `Animal with ID ${nonExistentAnimal} not found. Cannot generate report.`,
      );
    },
  );
  ...
  await t.step("should return an error if animal ID is missing", async () => {
    ...
    if (!isError(result)) {
      throw new Error(
        "Expected an error for missing animal ID, but generateReport succeeded unexpectedly.",
      );
    }
    assertExists(result.error);
    assertEquals(
      result.error,
      "Animal ID, report name, start date, and end date are required.",
    );
  });
  ...
  await t.step("should return an error if reportName is missing", async () => {
    ...
    if (!isError(result)) {
      throw new Error(
        "Expected an error for missing report name, but generateReport succeeded unexpectedly.",
      );
    }
    assertExists(result.error);
    assertEquals(
  ...
  await t.step(
    "should return an error if startDateRange is missing",
    async () => {
      const result = await concept.generateReport({
        animal: animalE,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing start date, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
      assertEquals(
  ...
  await t.step(
    "should return an error if endDateRange is missing",
    async () => {
      const result = await concept.generateReport({
        animal: animalE,
        startDateRange: d_jan1,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing end date, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
      assertEquals(
  ...
  await t.step(
    "should return an error if startDateRange is invalid",
    async () => {
      const result = await concept.generateReport({
        animal: animalE,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for invalid start date, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
  ...
  await t.step(
    "should return an error if endDateRange is invalid",
    async () => {
      const result = await concept.generateReport({
        animal: animalE,
        startDateRange: d_jan1,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for invalid end date, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
      assertEquals(result.error, "Invalid date range provided.");
  ...
  await t.step(
    "should return an error if startDateRange is after endDateRange",
    async () => {
      const result = await concept.generateReport({
        animal: animalE,
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for inverted date range, but generateReport succeeded unexpectedly.",
        );
      }
      assertExists(result.error);
...
Deno.test("GrowthTrackingConcept - renameReport action", async (t) => {
  ...
  if (isError(conflictReportResult)) {
    throw new Error(
      `Setup failed for conflict report: ${conflictReportResult.error}`,
    );
  }
  ...
  await t.step("should successfully rename an existing report", async () => {
    ...
    assertEquals(
      result.newName,
      newReportName,
      "Expected the new name to be returned",
    );

    // Verify old report name no longer exists
    const oldReportCheck = await concept._getReportByName({
      reportName: oldReportName,
    });
    if (!isError(oldReportCheck)) { // Expected an error because the report should be gone
      throw new Error(
        `Expected report '${oldReportName}' to be deleted, but it was found: ${
          JSON.stringify(oldReportCheck.report)
        }`,
      );
    }
    assertEquals(
      oldReportCheck.error,
      `Report with name '${oldReportName}' not found.`,
      "Old report name should no longer exist",
    );

    // Verify new report name exists and contains the correct data
    const newReportCheck = await concept._getReportByName({
      reportName: newReportName,
    });
    if (isError(newReportCheck)) { // Expected success here
      throw new Error(
        `Expected new report to exist, but got error: ${newReportCheck.error}`,
      );
    }
    ...
    assertEquals(
      ...
      newReportName,
      "Report should have the new name",
    );
    assertEquals(
      newReport.targetAnimals,
      [animalH],
      "Report should maintain target animals",
    );
  });

  await t.step(
    "should return an error if old report name is missing",
    async () => {
      const result = await concept.renameReport({
        oldName: undefined!,
        newName: "SomeNewName",
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing old name, but renameReport succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
        "Old report name and new report name are required.",
      );
    },
  );

  await t.step(
    "should return an error if new report name is missing",
    async () => {
      const result = await concept.renameReport({
        oldName: newReportName,
        newName: undefined!,
      });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing new name, but renameReport succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
  ...
  await t.step(
    "should return an error if the old report does not exist",
    async () => {
      const result = await concept.renameReport({
        oldName: "NonExistentReport",
        newName: "ReallyNewName",
      });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for non-existent old report, but renameReport succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
  ...
  await t.step(
    ...
    async () => {
      ...
      const result = await concept.renameReport({
        oldName: newReportName,
        newName: conflictingReportName,
      });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for conflicting new name, but renameReport succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
        `A report with name '${conflictingReportName}' already exists.`,
      );
    },
  );

  await t.step(
    "should return an error if old name and new name are the same",
    async () => {
      // Use the name 'ExistingReport' that we know exists
      const result = await concept.renameReport({
        oldName: conflictingReportName,
        newName: conflictingReportName,
      });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for same old/new name, but renameReport succeeded unexpectedly.",
        );
      }
...
Deno.test("GrowthTrackingConcept - deleteReport action", async (t) => {
  ...
  await t.step("should successfully delete an existing report", async () => {
    ...
    assertEquals(
      result,
      {},
      "Expected successful deleteReport to return an empty object",
    );

    // Verify the report no longer exists
    const deletedReportCheck = await concept._getReportByName({
      reportName: reportToDelete,
    });
    if (!isError(deletedReportCheck)) { // Expected an error because the report should be gone
      throw new Error(
        `Expected report '${reportToDelete}' to be deleted, but it was found: ${
          JSON.stringify(deletedReportCheck.report)
        }`,
      );
    }
    assertEquals(
      deletedReportCheck.error,
      `Report with name '${reportToDelete}' not found.`,
      "Deleted report should no longer be found",
    );
  });
  ...
  await t.step("should return an error if report name is missing", async () => {
    ...
    if (!isError(result)) {
      throw new Error(
        "Expected an error for missing report name, but deleteReport succeeded unexpectedly.",
      );
    }
  ...
  await t.step(
    "should return an error if the report does not exist",
    async () => {
      ...
      if (!isError(result)) {
        throw new Error(
          "Expected an error for non-existent report, but deleteReport succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
...
Deno.test("GrowthTrackingConcept - AI Features (aiSummary action, _getAiSummary query)", async (t) => {
  ...
  if (isError(addAnimalKResult)) {
    throw new Error(
      `Setup failed for adding animal K: ${addAnimalKResult.error}`,
    );
  }

  await t.step(
    "aiSummary action: should successfully generate and save an AI summary for an existing report",
    async () => {
      const aiSummaryResult = await concept.aiSummary({
        reportName: reportNameAI,
      });

      if (isError(aiSummaryResult)) {
        throw new Error(
          `Expected AI summary generation to succeed, but got error: ${aiSummaryResult.error}`,
        );
      }
      assertExists(
        aiSummaryResult.summary,
        "Expected a summary string in the result",
      );
      assertNotEquals(
        aiSummaryResult.summary,
        "",
      ...
      const fetchedReportResult = await concept._getReportByName({
        reportName: reportNameAI,
      });
      if (isError(fetchedReportResult)) {
        throw new Error(
          `Failed to fetch report after AI summary: ${fetchedReportResult.error}`,
        );
      }
      assertEquals(
        fetchedReportResult.report.aiGeneratedSummary,
        aiSummaryResult.summary,
        "Saved summary should match generated summary",
      );
      ...
      assertExists(
        ...
        "Parsed summary should have an 'insights' field",
      );
      assertEquals(
        typeof parsedSummary.insights,
        "string",
        "'insights' field should be a string",
      );
      assertExists(
        parsedSummary.highPerformers,
        "'highPerformers' field should exist",
      );
      assertEquals(
        Array.isArray(parsedSummary.highPerformers),
        true,
        "'highPerformers' should be an array",
      );
    },
  );

  await t.step(
    "aiSummary action: should return an error if report name is missing",
    async () => {
      const result = await concept.aiSummary({ reportName: undefined! });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing report name, but aiSummary succeeded unexpectedly.",
        );
      }
      assertEquals(result.error, "Report name is required.");
    },
  ...
  await t.step(
    "aiSummary action: should return an error if the report does not exist",
    async () => {
      const result = await concept.aiSummary({ reportName: nonExistentReport });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for non-existent report, but aiSummary succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
  ...
  await t.step(
    "_getAiSummary query: should return an existing summary without re-generating",
    async () => {
      // We already generated a summary for 'AISummaryReport' in the previous step
      const firstCallResult = await concept._getAiSummary({
        reportName: reportNameAI,
      });
      if (isError(firstCallResult)) {
        throw new Error(
          `Expected success on first call, but got error: ${firstCallResult.error}`,
        );
      }
      const initialSummary = firstCallResult.summary;

      const fetchedReportBeforeSecondCall = await concept._getReportByName({
        reportName: reportNameAI,
      });
      if (isError(fetchedReportBeforeSecondCall)) {
        throw new Error(
          `Failed to fetch report before second AI summary call: ${fetchedReportBeforeSecondCall.error}`,
        );
      }
      const initialDateGenerated =
        fetchedReportBeforeSecondCall.report.dateGenerated;

      // Call again, it should return the cached summary
      const secondCallResult = await concept._getAiSummary({
        reportName: reportNameAI,
      });
      if (isError(secondCallResult)) {
        throw new Error(
          `Expected success on second call, but got error: ${secondCallResult.error}`,
        );
      }
      assertEquals(
        secondCallResult.summary,
        initialSummary,
        "Second call should return the same cached summary",
      );
      ...
      const fetchedReportAfterSecondCall = await concept._getReportByName({
        reportName: reportNameAI,
      });
      if (isError(fetchedReportAfterSecondCall)) {
        throw new Error(
          `Failed to fetch report after second AI summary call: ${fetchedReportAfterSecondCall.error}`,
        );
      }
      assertEquals(
        fetchedReportAfterSecondCall.report.dateGenerated.getTime(),
        initialDateGenerated.getTime(),
        "Report 'dateGenerated' should not change for cached summary retrieval",
      );
    },
  );

  await t.step(
    "_getAiSummary query: should generate and save a new summary if none exists",
    async () => {
      ...
      if (isError(noAISetupResult)) {
        throw new Error(
          `Setup failed for report without AI summary: ${noAISetupResult.error}`,
        );
      }

      const reportBeforeGetAI = await concept._getReportByName({
        reportName: reportNameNoAI,
      });
      if (isError(reportBeforeGetAI)) {
        throw new Error(
          `Setup verification failed: ${reportBeforeGetAI.error}`,
        );
      }
      assertEquals(
        reportBeforeGetAI.report.aiGeneratedSummary,
        "",
        "New report should initially have no AI summary",
      );

      const getAiSummaryResult = await concept._getAiSummary({
        reportName: reportNameNoAI,
      });
      if (isError(getAiSummaryResult)) {
        throw new Error(
          `Expected _getAiSummary to generate, but got error: ${getAiSummaryResult.error}`,
        );
      }
      assertExists(
        getAiSummaryResult.summary,
        "Expected a summary to be generated and returned",
      );
      assertNotEquals(
        getAiSummaryResult.summary,
        "",
      ...
      // Verify it was saved
      const fetchedReportAfterGetAI = await concept._getReportByName({
        reportName: reportNameNoAI,
      });
      if (isError(fetchedReportAfterGetAI)) {
        throw new Error(
          `Post-generation fetch failed: ${fetchedReportAfterGetAI.error}`,
        );
      }
      assertEquals(
        fetchedReportAfterGetAI.report.aiGeneratedSummary,
        getAiSummaryResult.summary,
        "Generated summary should be saved to the report",
      );
    },
  );

  await t.step(
    "_getAiSummary query: should return an error if report name is missing",
    async () => {
      const result = await concept._getAiSummary({ reportName: undefined! });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for missing report name, but _getAiSummary succeeded unexpectedly.",
        );
      }
      assertEquals(result.error, "Report name is required.");
    },
  ...
  await t.step(
    "_getAiSummary query: should return an error if the report does not exist",
    async () => {
      const result = await concept._getAiSummary({
        reportName: nonExistentReport,
      });
      if (!isError(result)) {
        throw new Error(
          "Expected an error for non-existent report, but _getAiSummary succeeded unexpectedly.",
        );
      }
      assertEquals(
        result.error,
