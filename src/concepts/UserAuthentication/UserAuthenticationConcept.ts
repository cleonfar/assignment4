...
export default class UserAuthenticationConcept {
  ...
  async verify(
    ...
    if (typeof token !== "string") {
      console.error(
        "[AUTH-CONCEPT] verify(): invalid token type; expected string, got",
        typeof token,
      );
      return { error: "Invalid session token format: expected string" };
    }
    ...
    try {
      ...
      return {
        error: "Failed to verify session token due to a database error.",
      };
    }
  }
