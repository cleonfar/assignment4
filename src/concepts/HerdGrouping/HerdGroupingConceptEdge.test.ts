...
Deno.test("HerdGroupingConcept - Edge Cases", async (t) => {
  ...
  await t.step("createHerd for initial setup for edge cases", async () => {
    await concept.createHerd({
      name: HERD_E,
      description: "Initial setup herd E",
    });
    await concept.createHerd({
      name: HERD_F,
      description: "Initial setup herd F",
    });
    await concept.createHerd({
      name: HERD_G,
      description: "Initial setup herd G",
    });
    await concept.createHerd({
      name: HERD_H,
      description: "Initial setup herd H",
    });
    await concept.createHerd({
      name: HERD_I,
      description: "Initial setup herd I",
    });
    await concept.createHerd({
      name: HERD_J,
      description: "Initial setup herd J",
    });
  ...
  await t.step(
    ...
    async () => {
      ...
      if ("error" in initialHerdE) {throw new Error(
          `_viewComposition failed: ${initialHerdE.error}`,
        );}
      console.log(
        `HERD_E before split: ${JSON.stringify(initialHerdE.animals)}`,
      );
      ...
      assertEquals(
        "error" in result,
        false,
        `splitHerd failed unexpectedly: ${
          ("error" in result) ? result.error : ""
        }`,
      );

      // Verify NEW_HERD exists and has animals
      const newHerdComposition = await concept._viewComposition({
        herdName: NEW_HERD,
      });
      if ("error" in newHerdComposition) {
        throw new Error(
          `_viewComposition failed for new herd: ${newHerdComposition.error}`,
        );
      }
      assertArrayIncludes(
        newHerdComposition.animals,
        animalsToMove,
        `New herd '${NEW_HERD}' should contain moved animals.`,
      );
      assertEquals(newHerdComposition.animals.length, animalsToMove.length);
      console.log(
        `New Herd '${NEW_HERD}' after split: ${
          JSON.stringify(newHerdComposition.animals)
        }`,
      );

      // Verify animals removed from HERD_E
      const finalHerdE = await concept._viewComposition({ herdName: HERD_E });
      if ("error" in finalHerdE) {throw new Error(
          `_viewComposition failed: ${finalHerdE.error}`,
        );}
      ...
      assertEquals(
        list.herds.some((h) => h.name === NEW_HERD && !h.isArchived),
        true,
        `New herd '${NEW_HERD}' should be listed and not archived.`,
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      await concept.mergeHerds({
        herdNameToKeep: TEMP_HERD,
        herdNameToArchive: HERD_G,
      }); // Archive G

      let herdGStatus = await concept._listHerds();
      assertEquals(
        herdGStatus.herds.find((h) => h.name === HERD_G)?.isArchived,
        true,
        "HERD_G should be archived.",
      );
      ...
      console.log(
        `Move attempt from archived HERD_G resulted in error: ${result.error}`,
      );

      // Verify HERD_F remains unchanged
      const finalHerdF = await concept._viewComposition({ herdName: HERD_F });
      if ("error" in finalHerdF) {throw new Error(
          `_viewComposition failed: ${finalHerdF.error}`,
        );}
  ...
  await t.step(
    ...
    async () => {
      ...
      if ("error" in initialHerdF) {
        throw new Error(`_viewComposition failed: ${initialHerdF.error}`);
      }
      assertEquals(initialHerdF.animals.length, 1);
      assertArrayIncludes(initialHerdF.animals, [ANIMAL_E]);
      console.log(
        `HERD_F before removing last animal: ${
          JSON.stringify(initialHerdF.animals)
        }`,
      );
      ...
      assertEquals(
        "error" in result,
        false,
        `removeAnimal failed unexpectedly: ${
          ("error" in result) ? result.error : ""
        }`,
      );

      const finalHerdF = await concept._viewComposition({ herdName: HERD_F });
      if ("error" in finalHerdF) {
        throw new Error(`_viewComposition failed: ${finalHerdF.error}`);
      }
      assertEquals(
        finalHerdF.animals.length,
        0,
        "Herd should be empty after removing its last animal.",
      );
      console.log(
        `HERD_F after removing last animal: ${
          JSON.stringify(finalHerdF.animals)
        }`,
      );
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      if ("error" in initialHerdH) {
        throw new Error(`_viewComposition failed: ${initialHerdH.error}`);
      }
      assertArrayIncludes(initialHerdH.animals, animalsInH);
      assertEquals(initialHerdH.animals.length, animalsInH.length);
      console.log(
        `HERD_H before split: ${JSON.stringify(initialHerdH.animals)}`,
      );
      ...
      console.log(
        `HERD_F before split: ${JSON.stringify(initialHerdF.animals)}`,
      );
      ...
      if ("error" in finalHerdH) {
        throw new Error(`_viewComposition failed: ${finalHerdH.error}`);
      }
      assertEquals(
        finalHerdH.animals.length,
        0,
        "Source herd should be empty after moving all its animals.",
      );
      ...
      assertArrayIncludes(
        finalHerdF.animals,
        animalsInH,
        "Target herd should contain all moved animals.",
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      if ("error" in initialHerdI) {
        throw new Error(`_viewComposition failed: ${initialHerdI.error}`);
      }
      assertArrayIncludes(initialHerdI.animals, [ANIMAL_D, ANIMAL_E]);
      assertEquals(initialHerdI.animals.length, 2);
      console.log(
        `HERD_I (to keep) before merge: ${
          JSON.stringify(initialHerdI.animals)
        }`,
      );

      let initialHerdJ = await concept._viewComposition({ herdName: HERD_J });
      if ("error" in initialHerdJ) {
        throw new Error(`_viewComposition failed: ${initialHerdJ.error}`);
      }
      assertArrayIncludes(initialHerdJ.animals, [ANIMAL_E, ANIMAL_A]);
      assertEquals(initialHerdJ.animals.length, 2);
      console.log(
        `HERD_J (to archive) before merge: ${
          JSON.stringify(initialHerdJ.animals)
        }`,
      );
      ...
      assertEquals(
        "error" in result,
        false,
        `mergeHerds failed unexpectedly: ${
          ("error" in result) ? result.error : ""
        }`,
      );
      ...
      if ("error" in finalHerdI) {
        throw new Error(`_viewComposition failed: ${finalHerdI.error}`);
      }
      assertArrayIncludes(finalHerdI.animals, [ANIMAL_D, ANIMAL_E, ANIMAL_A]);
      assertEquals(
        finalHerdI.animals.length,
        3,
        "HERD_I should have unique animals from both, no duplicates.",
      );
      assertEquals(
        finalHerdI.animals.filter((a) => a === ANIMAL_E).length,
        1,
        "ANIMAL_E should not be duplicated in HERD_I.",
      );
      ...
      const finalHerdJComposition = await concept._viewComposition({
        herdName: HERD_J,
      });
      if ("error" in finalHerdJComposition) {
        throw new Error(
          `_viewComposition failed: ${finalHerdJComposition.error}`,
        );
      }
      assertEquals(
        finalHerdJComposition.animals.length,
        0,
        "Archived HERD_J should have its members cleared.",
      );
      console.log(
        `HERD_J after merge: ${JSON.stringify(finalHerdJComposition.animals)}`,
      );
    },
  );
